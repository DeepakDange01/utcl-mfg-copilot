...
const Chat = () => {
    ...
    const [hideErrorDialog, { toggle: toggleErrorDialog }] = useBoolean(true);
    const [errorMsg, setErrorMsg] = useState<ErrorMessage | null>()
    const [chatMode, setChatMode] = useState('KB'); // Default to 'KB'
    ...
    const ChatModeToggle = () => (
        <div className={styles.radioToolbar}>
            <input
                ...
                value="KB"
                checked={chatMode === 'KB'}
                onChange={() => setChatMode('KB')}
            />
            <label htmlFor="radioKB">Chat with Knowledge Base</label>
            ...
            />
            <label htmlFor="radioLLM">Chat with LLM</label>
        </div>
    );
    ...
    const makeApiRequestWithCosmosDB = async (question: string, chatMode: string, conversationId?: string ) => {
        setIsLoading(true);
        setShowLoadingMessage(true);
        const abortController = new AbortController();
        abortFuncs.current.unshift(abortController);
        ...
        let result = {} as ChatResponse;
        try {
            const response = conversationId ? await historyGenerate(request, abortController.signal, chatMode, conversationId ) : await historyGenerate(request, abortController.signal, chatMode);
            if(!response?.ok){
                let errorChatMsg: ChatMessage = {
                    id: uuid(),
                    role: ERROR,
                    ...
                    />
                        ...
                        <Stack horizontal className={styles.chatInput}>
                            ...
                            <QuestionInput
                                ...
                                disabled={isLoading}
                                onSend={(question, id) => {
                                    appStateContext?.state.isCosmosDBAvailable?.cosmosDB ? makeApiRequestWithCosmosDB(question, chatMode, id ) : makeApiRequestWithoutCosmosDB(question, id)
                                }}
                                conversationId={appStateContext?.state.currentChat?.id ? appStateContext?.state.currentChat?.id : undefined}
                            />
                        </Stack>
