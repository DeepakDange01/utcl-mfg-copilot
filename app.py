...
def should_use_data(chatMode: str):
    if chatMode == "LLM":
        return False
    #return False
    if AZURE_SEARCH_SERVICE and AZURE_SEARCH_INDEX and AZURE_SEARCH_KEY:
...
def prepare_body_headers_with_data(request):
    ...
    system_msg = {'id': 'system', 'role': 'system', 'content': AZURE_OPENAI_SYSTEM_MESSAGE}
...
def conversation_internal(request_body):
    ...
    chatMode = request_body.get("chatMode", "KB")
    try:
        use_data = should_use_data(chatMode)        
        if use_data:
            return conversation_with_data(request_body)
        else:
            return conversation_without_data(request_body)
...
def update_last_message_feedback():
    ...
    conversation_id = data['conversationId']
    is_positive = data['isPositive']
    text_feedback = data.get('textFeedback')  # Retrieve additional text feedback

    last_message = cosmos_conversation_client.get_last_message(user_id, conversation_id)
    if last_message:
        message_id = last_message['id']
        # Update message with additional text feedback
        updated_message = cosmos_conversation_client.update_message(user_id, conversation_id, message_id, is_positive, text_feedback)
        if updated_message:
            return {'status': 'success', 'messageId': updated_message['id']}
        else:
            return {'status': 'error'}, 500
...
def add_conversation():
    ...
    ## check request for conversation_id
    conversation_id = request.json.get("conversation_id", None)
    chatMode = request.json.get("chatMode", "KB")
    ...
    try:
        ...
        history_metadata['conversation_id'] = conversation_id
        request_body['history_metadata'] = history_metadata
        request_body['chatMode'] = chatMode
